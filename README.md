...
- Logging is controlled by `LOG_LEVEL` in `.env`.
- Supported levels: `silent`, `error`, `warn`, `info`, `debug`.
- Fallback behavior when `LOG_LEVEL` is not set or invalid:
- `NODE_ENV=production` -> `info`
- Non-production -> `debug`
- Full level reference: [`docs/logging-levels.md`](./docs/logging-levels.md)
