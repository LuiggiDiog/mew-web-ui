...
export function NewChatArea({ welcomeSeed = 0 }: NewChatAreaProps) {
  ...
  const handleSendImage = useCallback(
    async (prompt: string, width: number = imageWidth, height: number = imageHeight, referenceImage?: string, denoise?: number) => {
      ...
      try {
        ...
        const res = await fetch("/api/image", {
          ...
          signal: abortController.signal,
          body: JSON.stringify({
            prompt, width, height, chatHistory, preview: previewMode,
    ...
    [activeModel, streaming, router, setStreamingMessageId, imageWidth, imageHeight, previewMode, activeImageProfileId, messages]
  );
