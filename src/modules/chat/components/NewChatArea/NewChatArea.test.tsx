...
describe("NewChatArea image generation", () => {
  it("posts /api/image without conversationId and navigates to new conversation", async () => {
    ...
    await waitFor(() => {
      ...
      expect(imageCall?.[0]).toBe("/api/image");
      ...
      parsedBody = JSON.parse((imageCall?.[1] as RequestInit).body as string) as Record<string, unknown>;
    ...
    expect(parsedBody?.conversationId).toBeUndefined();
