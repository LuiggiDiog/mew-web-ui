    ...
    async (prompt: string, width: number = imageWidth, height: number = imageHeight, referenceImage?: string, denoise?: number) => {
      ...
      try {
        const chatHistory = messages
          .slice(-10)
          .map((m) => {
            if (m.type === "image" && m.role === "assistant") {
              return { role: m.role, content: "[Image generated]" };
            }
            if (m.role === "user" && m.content.startsWith("[[ref:")) {
              const endIdx = m.content.indexOf("]]");
              const text = endIdx !== -1 ? m.content.slice(endIdx + 2).trim() : m.content;
